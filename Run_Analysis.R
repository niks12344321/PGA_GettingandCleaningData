...
RawDatDir <- "./rawData"
RawDatURL <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
RawDatFile <- "rawData.zip"
rawDataDFn <- paste(RawDatDir, "/", "rawData.zip", sep = "")
dataDir <- "./data"

if (!file.exists(RawDatDir)) {
  dir.create(RawDatDir)
  download.file(url = RawDatURL, destfile = rawDataDFn)
}
...
#5. final Tidy datatset
TempDat <- melt(allData, id = c("Subject", "Activity"))
tidyData <- dcast(TempDat, Subject + Activity ~ variable, mean)

write.table(tidyData, "./Final_tidy_dataset.txt", row.names = FALSE, quote = FALSE)
